...
import com.extensions.logmonitor.jsonLogModule.jsonLogSqlParseies.jsonLogSqlParser.Math_functionsContext;
...
public class JsonLogSqlAnalyzer extends jsonLogSqlBaseListener {
	...
	private TwoTuple<ValueConvert, List<String>> doHandleFunctionCall(Function_callContext function_call) {
		...
		Math_functionsContext math_functions = functionList.math_functions();
		String functionName = "";
		...
		} else if (math_functions != null) {
			type = FunctionCreator.FUN_MATH_TYPE;
			if (math_functions.RANDOM() != null) {
				functionName = "RANDOM";
			}
		}
	...
	public void exitSelectTableColumn(SelectTableColumnContext ctx) {
		...
		if (this.currentScope instanceof SelectScope) {
			SelectPart selectPart = ((SelectScope) this.currentScope).getSelectPart();
			QueryExecute<? extends Object> queryExecute = new ColumnValueQueryExecute();
			queryExecute.setQueryPath(ctx.array_column_spec().getText());
